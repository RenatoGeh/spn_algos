...
\begin{algorithm}[h]
  ...
  \begin{algorithmic}[1]
    ...
    \State\,Let $S=(V,E)$, where $V$ is the set of vertices and $E$ the set of edges of $S$
    \State\,Let $Q$ be a queue data structure
    \State\,Let $T$ be a stack data structure
    \State\,Let $U$ be a list of booleans, where $U[i]$ is true if node $i$ has already been
      visited
    \State\,Let $L$ be an adjacency list where $L[i]$ contains a list of the parents of node $i$
    \State\,\mcode{$Q$.Enqueue$(S)$}, \mcode{$T$.Push$(S)$}
    \State\,$U[S]\gets\textbf{true}$, $L[S]\gets\textbf{nil}$
