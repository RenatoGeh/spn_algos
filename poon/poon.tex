...
We denote $e_{i,j}$ as an edge that goes from node $i$ to node $j$. We denote $\Val(e_{i,j})$ as
the weight value of edge $e_{i,j}$. In~\autoref{alg:prunezeroweights}, we traverse the graph of SPN
$S$ in a bottom-up fashion. We run through the graph this particular way to avoid recomputations.
By going bottom-up, at every zero weight edge of a sum node $s$ we encounter, we can guarantee that
...
weight edge in the graph scope of the function. Therefore, it suffices to recursively delete
orphan nodes.
